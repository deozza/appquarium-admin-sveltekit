...
<div class="flex-c space-y-6">
	...
	{:else}
		...
		<section class="w-3/5 flex-c space-y-6 p-6 bg-white border-2 rounded-md border-black">
			...
			<GeneralForm species={plant} speciesOrigins={$origins} {user} />
		</section>
		...
		<section class="w-3/5 flex-c space-y-6 p-6 bg-white border-2 rounded-md border-black">
			...
			<NamingForm
				species={plant}
				speciesFamilies={$plantFamilies}
				speciesGenres={$plantGenres}
				{user}
			/>
		</section>
		...
		<section class="w-3/5 flex-c space-y-6 p-6 bg-white border-2 rounded-md border-black">
			...
			<WaterConstraintsForm species={plant} {user} />
		</section>

		<section class="w-3/5 flex-c space-y-6 p-6 bg-white border-2 rounded-md border-black">
			<BaseHeader baseHeaderModel={specsFormHeader} />
			<PlantSpecsForm species={plant} {user} growthSpeeds={$growthSpeeds} zones={$plantZones} />
		</section>
		...
		<section class="w-3/5 flex-c space-y-6 p-6 bg-white border-2 rounded-md border-black">
			<PublicationStateSwitcher species={plant} {user} />
		</section>
	{/if}
</div>
