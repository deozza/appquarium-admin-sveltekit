			...
			<ul class="space-y-2">
					...
					<div class="w-full flex-r">
						...
						<div style="flex: 2">
							<input
								...
								on:change={() => handleThumbnailClick(image)}
								id="updateFileThumbnailLabel"
							/>
						</div>
					</div>
				</li>
				<li class="flex-c">
					<div class="w-full flex-r justify-around">
						<BaseButton
							baseButtonModel={formElements['updateButton']}
							on:click={() => editFileMetadata(image)}
						/>
						<BaseButton
							baseButtonModel={formElements['deleteButton']}
							on:click={() => deleteFile(image)}
						/>
					</div>
				</li>
			</ul>
