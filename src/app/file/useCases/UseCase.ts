...
export default class UseCase implements UseCaseInterface {
	async uploadFile(
		...
		const image: Image | Array<UseCaseError> = await fileService.uploadFile(
			completeRemotePath,
			file
		);
	...
	async editFileMetadata(
		jwt: string,
		image: Image,
		thumbnailHasBeenUpdated: boolean
	): Promise<Result> {
		const result: Result = new Result();
		const fileService: Service = new Service();

		if (thumbnailHasBeenUpdated === true && image.thumbnail === true) {
			const cleanThumbnailBeforeEdit: boolean | Array<UseCaseError> =
				await fileService.cleanThumbnailBeforeEdit(jwt, image);
