...
export default class InvertebrateUseCase implements UseCaseInterface {
	async getListOfInvertebrates(jwt: string): Promise<Result> {
		...
		const listOfInvertebrates: Array<Species> | UseCaseError =
			await invertebrateService.queryGetListOfInvertebrates(jwt);
	...
	async getInvertebrateGenres(jwt: string): Promise<Result> {
		...
		const invertebrateGenres: Array<SpeciesGenre> | UseCaseError =
			await invertebrateService.queryInvertebrateGenres(jwt);
	...
	async getInvertebrateFamilies(jwt: string): Promise<Result> {
		...
		const invertebrateFamilies: Array<SpeciesFamily> | UseCaseError =
			await invertebrateService.queryInvertebrateFamilies(jwt);
