...
export default class Services implements ServicesInterface {
	...
	async createSpeciesFamily(
		jwt: string,
		speciesFamily: SpeciesFamily
	): Promise<string | UseCaseError> {
	...
	async createSpeciesGenre(
		jwt: string,
		speciesGenre: SpeciesGenre
	): Promise<string | UseCaseError> {
	...
	async createSpeciesNaming(
		jwt: string,
		speciesNaming: SpeciesNaming
	): Promise<string | UseCaseError> {
	...
	async addNamingToSpecies(
		jwt: string,
		speciesNaming: SpeciesNaming
	): Promise<SpeciesNaming | UseCaseError> {
		const adapter: SpeciesNamingAdapterInterface = new SpeciesNamingHasuraAdapter(jwt);

		return await adapter.mutationAddNamingToSpecies(speciesNaming);
	}

	async updateSpeciesNaming(
		jwt: string,
		speciesNaming: SpeciesNaming
	): Promise<string | UseCaseError> {
		const adapter: SpeciesNamingAdapterInterface = new SpeciesNamingHasuraAdapter(jwt);
	...
	async addWaterConstraintsToSpecies(
		jwt: string,
		waterConstraints: WaterConstraints
	...
	async createAnimalSpecs(
		jwt: string,
		animalSpecs: AnimalSpecs
	): Promise<string | Array<UseCaseError>> {
	...
	async addAnimalSpecsToSpecies(
		jwt: string,
		animalSpecs: AnimalSpecs
	): Promise<AnimalSpecs | UseCaseError> {
		const adapter: AnimalSpecsAdapterInterface = new AnimalSpecsHasuraAdapter(jwt);

		return await adapter.mutationAddAnimalSpecsToSpecies(animalSpecs);
	}

	async updateAnimalSpecs(
		jwt: string,
		animalSpecs: AnimalSpecs
	): Promise<AnimalSpecs | Array<UseCaseError>> {
		const adapter: AnimalSpecsAdapterInterface = new AnimalSpecsHasuraAdapter(jwt);

		return await adapter.mutationEditAnimalSpecs(animalSpecs);
	}

	async createPlantSpecs(
		jwt: string,
	...
	async addPlantSpecsToSpecies(
		jwt: string,
		plantSpecs: PlantSpecs
	): Promise<PlantSpecs | UseCaseError> {
		const adapter: PlantSpecsAdapterInterface = new PlantSpecsHasuraAdapter(jwt);

		return await adapter.mutationAddPlantSpecsToSpecies(plantSpecs);
	}

	async updatePlantSpecs(
		jwt: string,
		plantSpecs: PlantSpecs
	): Promise<PlantSpecs | Array<UseCaseError>> {
		const adapter: PlantSpecsAdapterInterface = new PlantSpecsHasuraAdapter(jwt);

		return await adapter.mutationEditPlantSpecs(plantSpecs);
	}

	async createAnimalBehaviour(
		jwt: string,
	...
	async addAnimalBehaviourToSpecies(
		jwt: string,
		animalBehaviour: AnimalBehaviour
	): Promise<AnimalBehaviour | UseCaseError> {
		const adapter: AnimalBehaviourAdapterInterface = new AnimalBehaviourHasuraAdapter(jwt);

		return await adapter.mutationAddAnimalBehaviourToSpecies(animalBehaviour);
	}

	async updateAnimalBehaviour(
		jwt: string,
		animalBehaviour: AnimalBehaviour
	): Promise<AnimalBehaviour | Array<UseCaseError>> {
		const adapter: AnimalBehaviourAdapterInterface = new AnimalBehaviourHasuraAdapter(jwt);

		return await adapter.mutationEditAnimalBehaviour(animalBehaviour);
	}

	async createAquariumConstraints(
		jwt: string,
	...
	async addAquariumConstraintsToSpecies(
		jwt: string,
		aquariumConstraints: AquariumConstraints
	...
	async updateAquariumConstraints(
		jwt: string,
		aquariumConstraints: AquariumConstraints
	...
	async checkNextState(
		species: Species,
		nextState: string
	): Promise<boolean | Array<UseCaseError>> {
		return true;
	}
