...
export default class HasuraAdapter extends HasuraClient implements AdapterInterface {
	async queryTotalSpecies(speciesConstraints: Constraints): Promise<number | null> {
		...
		const speciesAggregateSubQuery: Query = new Query('species_aggregate').addReturnToQuery(
			new Query('aggregate').addReturnToQuery('count')
		);
	...
	async queryListOfSpecies(
		speciesConstraints: Constraints
	): Promise<Array<Species> | UseCaseError> {
		const queryBuilder: Query = new Query('query');
		...
		const speciesSubQuery: Query = new Query('species')
			...
			.addReturnToQuery(
				new Query('naming')
					...
					.addReturnToQuery(new Query('species_genre').addReturnToQuery('name'))
			);

		speciesConstraints.orderBy = new ConstraintPart('order_by').addConstraint([
		...
		try {
			const data = await this.client.request(query);
			const listOfSpecies: Array<Species> = data.species.map(
				(item: Array<string>) => new Species(item)
			);
	...
	async queryGetSpecies(uuid: string): Promise<Species | UseCaseError> {
		...
		const speciesByPkSubQuery: Query = new Query('species_by_pk')
			...
			.addReturnToQuery(
				new Query('naming')
					...
					.addReturnToQuery(
						new Query('species_family').addReturnToQuery('name').addReturnToQuery('uuid')
					)
					.addReturnToQuery(
						new Query('species_genre').addReturnToQuery('name').addReturnToQuery('uuid')
					)
			)
			.addReturnToQuery(
			...
			.addReturnToQuery(
				new Query('species_aquarium_constraints')
					.addReturnToQuery('uuid')
					.addReturnToQuery('created_at')
					.addReturnToQuery('updated_at')
					.addReturnToQuery('species_uuid')
		...
		speciesByPkSubQuery.constraints.where = new ConstraintPart('uuid').addConstraint(
			'"' + uuid + '"'
		);
	...
	async queryListOfSpeciesCategories(): Promise<Array<string> | UseCaseError> {
		const queryBuilder: Query = new Query('query').addReturnToQuery(
			new Query('species_categories').addReturnToQuery('name')
		);
	...
	async queryListOfSpeciesOrigins(): Promise<Array<string> | UseCaseError> {
		const queryBuilder: Query = new Query('query').addReturnToQuery(
			new Query('species_origin').addReturnToQuery('name')
		);
	...
	async mutationCreateSpecies(species: Species): Promise<string | UseCaseError> {
		...
		const insertSpeciesOneSubQuery: Query = new Query('insert_species_one').addReturnToQuery(
			'uuid'
		);

		insertSpeciesOneSubQuery.constraints = new Constraints();
		insertSpeciesOneSubQuery.constraints.set = new ConstraintPart('object').addConstraint([
	...
	async mutationUpdatePublicationState(
		uuid: string,
		nextState: string
	...
	async mutationDeleteSpecies(uuid: string): Promise<boolean | Array<UseCaseError>> {
		const queryBuilder: Query = new Query('mutation');
		const deleteSpeciesByPkSubQuery: Query = new Query('delete_species_by_pk').addReturnToQuery(
			'uuid'
		);

		deleteSpeciesByPkSubQuery.constraints = new Constraints();
		deleteSpeciesByPkSubQuery.constraints.where = new ConstraintPart('pk_columns').addConstraint([
			new ConstraintPart('uuid').addConstraint('"' + uuid + '"')
		]);
