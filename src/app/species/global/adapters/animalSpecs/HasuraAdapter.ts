...
export default class AnimalSpecsHasuraAdapter
	extends HasuraClient
	implements AnimalSpecsAdapterInterface
{
	async mutationCreateAnimalSpecs(animalSpecs: AnimalSpecs): Promise<string | Array<UseCaseError>> {
		...
		const insertAnimalSpecsOneSubQuery: Query = new Query(
			'insert_animal_specs_one'
		).addReturnToQuery('uuid');

		insertAnimalSpecsOneSubQuery.constraints = new Constraints();
		insertAnimalSpecsOneSubQuery.constraints.set = new ConstraintPart('object').addConstraint([
			...
			new ConstraintPart('longevity_in_years').addConstraint(
				animalSpecs.longevity_in_years.toString()
			),
	...
	async mutationAddAnimalSpecsToSpecies(
	...
	async mutationEditAnimalSpecs(
		animalSpecs: AnimalSpecs
	): Promise<AnimalSpecs | Array<UseCaseError>> {
		const queryBuilder: Query = new Query('mutation');
		const updateAnimalSpecsByPkSubQuery: Query = new Query(
			'update_animal_specs_by_pk'
		).addReturnToQuery('uuid');

		updateAnimalSpecsByPkSubQuery.constraints = new Constraints();
		updateAnimalSpecsByPkSubQuery.constraints.where = new ConstraintPart(
			'pk_columns'
		).addConstraint([new ConstraintPart('uuid').addConstraint('"' + animalSpecs.user_uid + '"')]);

		updateAnimalSpecsByPkSubQuery.constraints.set = new ConstraintPart('_set').addConstraint([
			new ConstraintPart('male_size').addConstraint('"' + animalSpecs.male_size + '"'),
			new ConstraintPart('female_size').addConstraint('"' + animalSpecs.female_size + '"'),
			new ConstraintPart('longevity_in_years').addConstraint(
				animalSpecs.longevity_in_years.toString()
