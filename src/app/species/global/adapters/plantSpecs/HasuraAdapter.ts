...
export default class PlantSpecsHasuraAdapter extends HasuraClient implements PlantSpecsAdapterInterface {
	async mutationCreatePlantSpecs(plantSpecs: PlantSpecs): Promise<string | Array<UseCaseError>> {
		...
		const insertPlantSpecsOneSubQuery: Query = new Query('insert_plant_specs_one')
			.addReturnToQuery('uuid')

		insertPlantSpecsOneSubQuery.constraints = new Constraints()
		insertPlantSpecsOneSubQuery.constraints.set = new ConstraintPart('object')
			.addConstraint([
				...
				new ConstraintPart('need_in_fertilizer').addConstraint(plantSpecs.need_in_fertilizer.toString()),
	...
	async mutationEditPlantSpecs(plantSpecs: PlantSpecs): Promise<PlantSpecs | Array<UseCaseError>> {
		const queryBuilder: Query = new Query('mutation')
		const updatePlantSpecsByPkSubQuery: Query = new Query('update_plant_specs_by_pk')
			.addReturnToQuery('uuid')

		updatePlantSpecsByPkSubQuery.constraints = new Constraints()
		updatePlantSpecsByPkSubQuery.constraints.where = new ConstraintPart('pk_columns')
			.addConstraint([
				new ConstraintPart('uuid').addConstraint('"' + plantSpecs.user_uid + '"')
			])

		updatePlantSpecsByPkSubQuery.constraints.set = new ConstraintPart('_set')
			.addConstraint([
				new ConstraintPart('size').addConstraint(plantSpecs.size.toString()),
				new ConstraintPart('zone').addConstraint(plantSpecs.zone),
				new ConstraintPart('growth_speed').addConstraint(plantSpecs.growth_speed),
