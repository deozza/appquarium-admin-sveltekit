...
export default class AquariumConstraintsHasuraAdapter
	extends HasuraClient
	implements AquariumConstraintsAdapterInterface
{
	async queryListOfDecors(): Promise<Array<string> | UseCaseError> {
		const queryBuilder: Query = new Query('query').addReturnToQuery(
			new Query('decor_element').addReturnToQuery('name')
		);
	...
	async queryListOfSoilKinds(): Promise<Array<string> | UseCaseError> {
		const queryBuilder: Query = new Query('query').addReturnToQuery(
			new Query('soil_kind').addReturnToQuery('name')
		);
	...
	async mutationCreateAquariumConstraints(
		aquariumConstraints: AquariumConstraints
	): Promise<string | Array<UseCaseError>> {
		const queryBuilder: Query = new Query('mutation');
		const insertAquariumConstraintsOneSubQuery: Query = new Query(
			'insert_species_aquarium_constraints_one'
		...
		insertAquariumConstraintsOneSubQuery.constraints.set = new ConstraintPart(
			'object'
	...
	async mutationAddAquariumConstraintsToSpecies(
		aquariumConstraints: AquariumConstraints
		...
		updateSpeciesByPkSubQuery.constraints.set = new ConstraintPart('_set').addConstraint([
			new ConstraintPart('aquarium_constraints_uuid').addConstraint(
				'"' + aquariumConstraints.uuid + '"'
			)
		]);
	...
	async mutationEditAquariumConstraints(
		aquariumConstraints: AquariumConstraints
	): Promise<AquariumConstraints | Array<UseCaseError>> {
		const queryBuilder: Query = new Query('mutation');
		const updateAquariumConstraintsByPkSubQuery: Query = new Query(
			'update_species_aquarium_constraints_by_pk'
		).addReturnToQuery('uuid');

		updateAquariumConstraintsByPkSubQuery.constraints = new Constraints();
		updateAquariumConstraintsByPkSubQuery.constraints.where = new ConstraintPart(
			'pk_columns'
		).addConstraint([
			new ConstraintPart('uuid').addConstraint('"' + aquariumConstraints.uuid + '"')
		]);
		updateAquariumConstraintsByPkSubQuery.constraints.set = new ConstraintPart(
			'_set'
		).addConstraint([
			new ConstraintPart('min_volume').addConstraint(aquariumConstraints.min_volume.toString()),
			new ConstraintPart('max_volume').addConstraint(aquariumConstraints.max_volume.toString()),
