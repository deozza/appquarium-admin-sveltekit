...
export default class SpeciesNamingHasuraAdapter
	extends HasuraClient
	implements SpeciesNamingAdapterInterface
{
	async mutationCreateNaming(speciesNaming: SpeciesNaming): Promise<string | UseCaseError> {
		...
		const insertSpeciesNamingOneSubQuery: Query = new Query(
			'insert_species_naming_one'
		).addReturnToQuery('uuid');

		insertSpeciesNamingOneSubQuery.constraints = new Constraints();
		insertSpeciesNamingOneSubQuery.constraints.set = new ConstraintPart('object').addConstraint([
			...
			new ConstraintPart('family_uuid').addConstraint(
				'"' + speciesNaming.species_family.uuid + '"'
			),
	...
	async mutationAddNamingToSpecies(
	...
	async mutationUpdateSpeciesNaming(speciesNaming: SpeciesNaming): Promise<string | UseCaseError> {
		const queryBuilder: Query = new Query('mutation');
		const updateSpeciesNamingByPkSubQuery: Query = new Query(
			'update_species_naming_by_pk'
		).addReturnToQuery('uuid');

		updateSpeciesNamingByPkSubQuery.constraints = new Constraints();
		updateSpeciesNamingByPkSubQuery.constraints.where = new ConstraintPart(
			'pk_columns'
		).addConstraint([new ConstraintPart('uuid').addConstraint('"' + speciesNaming.uuid + '"')]);

		updateSpeciesNamingByPkSubQuery.constraints.set = new ConstraintPart('_set').addConstraint([
			new ConstraintPart('genre_uuid').addConstraint('"' + speciesNaming.species_genre.uuid + '"'),
			new ConstraintPart('family_uuid').addConstraint(
				'"' + speciesNaming.species_family.uuid + '"'
			),
