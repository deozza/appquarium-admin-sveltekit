...
export default class SpeciesFamiliesHasuraAdapter
	extends HasuraClient
	implements SpeciesFamiliesAdapterInterface
{
	async queryListOfSpeciesFamiliesByCategory(
		category: string
	): Promise<Array<SpeciesFamily> | UseCaseError> {
		...
		speciesFamilySubQuery.constraints.where = new ConstraintPart('where').addConstraint([
		...
		try {
			...
			const listOfSpeciesFamilies: Array<SpeciesFamily> = data.species_family.map(
				(item: Array<string>) => new SpeciesFamily(item)
			);
	...
	async mutationCreateSpeciesFamily(speciesFamily: SpeciesFamily): Promise<string | UseCaseError> {
		...
		const insertSpeciesFamilyOneSubQuery: Query = new Query(
			'insert_species_family_one'
		).addReturnToQuery('uuid');

		insertSpeciesFamilyOneSubQuery.constraints = new Constraints();
		insertSpeciesFamilyOneSubQuery.constraints.set = new ConstraintPart('object').addConstraint([
