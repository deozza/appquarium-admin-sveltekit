...
export default class SpeciesGenresHasuraAdapter
	extends HasuraClient
	implements SpeciesGenresAdapterInterface
{
	async queryListOfSpeciesGenresByCategory(
		category: string
	): Promise<Array<SpeciesGenre> | UseCaseError> {
		...
		try {
			...
			const listOfSpeciesGenres: Array<SpeciesGenre> = data.species_genre.map(
				(item: Array<string>) => new SpeciesGenre(item)
			);
	...
	async mutationCreateSpeciesGenre(speciesGenre: SpeciesGenre): Promise<string | UseCaseError> {
		...
		const insertSpeciesGenreOneSubQuery: Query = new Query(
			'insert_species_genre_one'
		).addReturnToQuery('uuid');

		insertSpeciesGenreOneSubQuery.constraints = new Constraints();
		insertSpeciesGenreOneSubQuery.constraints.set = new ConstraintPart('object').addConstraint([
