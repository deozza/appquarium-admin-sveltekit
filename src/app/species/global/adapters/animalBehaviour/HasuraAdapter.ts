...
export default class AnimalBehaviourHasuraAdapter
	extends HasuraClient
	implements AnimalBehaviourAdapterInterface
{
	async queryListOfAlimentations(): Promise<Array<string> | UseCaseError> {
		const queryBuilder: Query = new Query('query').addReturnToQuery(
			new Query('alimentation').addReturnToQuery('name')
		);
	...
	async queryListOfAnimalZones(): Promise<Array<string> | UseCaseError> {
		const queryBuilder: Query = new Query('query').addReturnToQuery(
			new Query('animal_zone').addReturnToQuery('name')
		);
	...
	async queryListOfAquariumKinds(): Promise<Array<string> | UseCaseError> {
		const queryBuilder: Query = new Query('query').addReturnToQuery(
			new Query('aquarium_kind').addReturnToQuery('name')
		);
	...
	async queryListOfBehaviours(): Promise<Array<string> | UseCaseError> {
		const queryBuilder: Query = new Query('query').addReturnToQuery(
			new Query('behaviour').addReturnToQuery('name')
		);
	...
	async mutationCreateAnimalBehaviour(
		animalBehaviour: AnimalBehaviour
	): Promise<string | Array<UseCaseError>> {
		const queryBuilder: Query = new Query('mutation');
		const insertAnimalBehaviourOneSubQuery: Query = new Query(
			'insert_animal_behaviour_one'
	...
	async mutationAddAnimalBehaviourToSpecies(
		animalBehaviour: AnimalBehaviour
	...
	async mutationEditAnimalBehaviour(
		animalBehaviour: AnimalBehaviour
	): Promise<AnimalBehaviour | Array<UseCaseError>> {
		const queryBuilder: Query = new Query('mutation');
		const updateAnimalBehaviourByPkSubQuery: Query = new Query(
			'update_animal_behaviour_by_pk'
		).addReturnToQuery('uuid');

		updateAnimalBehaviourByPkSubQuery.constraints = new Constraints();
		updateAnimalBehaviourByPkSubQuery.constraints.where = new ConstraintPart(
			'pk_columns'
		).addConstraint([new ConstraintPart('uuid').addConstraint('"' + animalBehaviour.uuid + '"')]);
		updateAnimalBehaviourByPkSubQuery.constraints.set = new ConstraintPart('_set').addConstraint([
			new ConstraintPart('aquarium_kind').addConstraint(animalBehaviour.aquarium_kind),
			new ConstraintPart('intraspecific_behaviour').addConstraint(
				animalBehaviour.intraspecific_behaviour
			),
