...
export default class WaterConstraintsHasuraAdapter
	extends HasuraClient
	implements WaterConstraintsAdapterInterface
{
	async mutationCreateWaterConstraints(
		waterConstraints: WaterConstraints
	): Promise<string | Array<UseCaseError>> {
		const queryBuilder: Query = new Query('mutation');
		const insertWaterConstraintsOneSubQuery: Query = new Query(
			'insert_water_constraints_one'
		).addReturnToQuery('uuid');

		insertWaterConstraintsOneSubQuery.constraints = new Constraints();
		insertWaterConstraintsOneSubQuery.constraints.set = new ConstraintPart('object').addConstraint([
	...
	async mutationAddWaterConstraintsToSpecies(
		waterConstraints: WaterConstraints
	...
	async mutationEditWaterConstraints(
		waterConstraints: WaterConstraints
	): Promise<WaterConstraints | Array<UseCaseError>> {
		const queryBuilder: Query = new Query('mutation');
		const updateWaterConstraintsByPkSubQuery: Query = new Query(
			'update_water_constraints_by_pk'
		).addReturnToQuery('uuid');

		updateWaterConstraintsByPkSubQuery.constraints = new Constraints();
		updateWaterConstraintsByPkSubQuery.constraints.where = new ConstraintPart(
			'pk_columns'
		).addConstraint([new ConstraintPart('uuid').addConstraint('"' + waterConstraints.uuid + '"')]);
		updateWaterConstraintsByPkSubQuery.constraints.set = new ConstraintPart('_set').addConstraint([
			new ConstraintPart('ph_min').addConstraint('"' + waterConstraints.ph_min + '"'),
			new ConstraintPart('ph_max').addConstraint('"' + waterConstraints.ph_max + '"'),
			new ConstraintPart('gh_min').addConstraint(waterConstraints.gh_min.toString()),
			new ConstraintPart('gh_max').addConstraint(waterConstraints.gh_max.toString()),
