...
export default class PlantUseCase implements UseCaseInterface {
	async getListOfPlants(jwt: string): Promise<Result> {
		...
		const listOfPlants: Array<Species> | UseCaseError = await plantService.queryGetListOfPlants(
			jwt
		);
	...
	async getListOfGrowthSpeeds(jwt: string): Promise<Result> {
		...
		const listOfGrowthSpeeds: Array<string> | UseCaseError =
			await plantService.queryGetListOfGrowthSpeeds(jwt);
	...
	async getPlantGenres(jwt: string): Promise<Result> {
		...
		const plantGenres: Array<SpeciesGenre> | UseCaseError = await plantService.queryPlantGenres(
			jwt
		);
	...
	async getPlantFamilies(jwt: string): Promise<Result> {
		...
		const plantFamilies: Array<SpeciesFamily> | UseCaseError =
			await plantService.queryPlantFamilies(jwt);
